#!/bin/bash
# Reset Home Assistant to a clean state (keeps configuration.yaml unless asked)
...
[ -L custom_components ] && rm custom_components && echo -e "${GREEN}✓ Removed custom_components symlink${NC}"
...
# Ask user if they want to restore the default configuration
echo -e "${YELLOW}Restore configuration.yaml?${NC}"
echo "  1) Keep current configuration.yaml (backed up to /tmp/configuration.yaml.backup)"
echo "  2) Restore default Aseko Local development configuration"
echo ""
read -p "Choose option [1-2] (default: 1): " RESTORE_CHOICE
...
EOF
        echo -e "${GREEN}✓ Restored default configuration${NC}"
        ;;
    *)
...
echo "3. Create a new user account / complete onboarding"
echo ""
...
echo "  - deps/, .cache/, blueprints/, tts/, .cloud/"
echo "  - custom_components/ (symlink, recreated by start-ha.sh)"
echo ""
